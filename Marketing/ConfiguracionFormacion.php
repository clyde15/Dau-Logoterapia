...
<div align="center">
    ...
    if ($Curso == 1){
     ?>
    <p align="center"><H3>Formaciones En Curso</H3></p>     
    <?php
    }else{
        ?>
    <p align="center"><H3>Formaciones Activos</H3></p>     
    <?php
    }
    ?>
...
<div class="clearfloat"></div>
      ...
      <table style="font: small" class="tablesorter table table-striped table-hover table-bordered table-condensed  " >  
        ...
        <tr >
           ...
           <a style="color:black" onclick="GETConfigurarFormacion()" ><img title="Configurar Nueva Formacion" alt="" SRC="../wp-content/uploads/2015/10/AdmDipomado.png" width="50px"/></a>
...
<!--            <th>Tipo</th>
   ...
   <tbody class="buscar">
        <?php
        if ($Curso == 1){
            $query = "SELECT C.IDConfiguracion, F.Nombre, F.Descripcion, DATE(C.FechaInicio) as FechaInicio, DATE(C.FechaFin) as FechaFin, C.CantidadModulos,C.CupoMinimo,C.EnCurso,T.Nombre as Tipo, U.Nombre as profinicial, U2.nombre as GerenteResponsable  FROM configuraciones As C INNER JOIN Formacion AS F ON C.ID_Formacion = F.idformaciones INNER JOIN Tipo AS T ON C.ID_Tipo = T.idtipo INNER JOIN Profesores As U on C.ID_ProfesorInicial = U.idprofesores INNER JOIN Usuarios As U2 on C.ID_GerenteResponsable = U2.idusuarios WHERE (C.Activo = 1) AND (C.EnCurso=1)";
             $resultados = $conexion->query($query);
        }  else {
            $query = "SELECT C.IDConfiguracion, F.Nombre, F.Descripcion, DATE(C.FechaInicio) as FechaInicio, DATE(C.FechaFin) as FechaFin, C.CantidadModulos,C.CupoMinimo,C.EnCurso,T.Nombre as Tipo, U.Nombre as profinicial, U2.nombre as GerenteResponsable  FROM configuraciones As C INNER JOIN Formacion AS F ON C.ID_Formacion = F.idformaciones INNER JOIN Tipo AS T ON C.ID_Tipo = T.idtipo INNER JOIN Profesores As U on C.ID_ProfesorInicial = U.idprofesores INNER JOIN Usuarios As U2 on C.ID_GerenteResponsable = U2.idusuarios WHERE (C.Activo = 1)";
             $resultados = $conexion->query($query);
        }
     ...
     ?>
      <tr>
         ...
         <td> <?php echo $registros['CupoMinimo'] ?>  </td>
         <?php
...
<!--        <td> <?php echo $registros['Tipo'] ?>  </td>
        ...
        <?php 
         if ($registros['EnCurso'] == false)
         {
          ...
          ?>
           <td>
                <a style="color:black" onclick="IniciarDiplomado(<?php echo $iddiplomado ?>,<?php echo $registros['EnCurso'] ?>,<?php echo $registrosinsc['inscritos'] ?>,<?php echo $registros['CupoMinimo'] ?>,<?php echo $solicitud; ?> )" type="button"><img title="Iniciar Formacion" alt="" SRC="../wp-content/uploads/2015/10/iniciar-la-reproduccion.png" width="30px"/></a>
            </td>         
         <?php
         }else{
             ?>
           <td>
                <a style="color:black" onclick="Detener(<?php echo $iddiplomado ?>,<?php echo $registros['EnCurso'] ?>)" type="button"><img title="Detener Formacion " alt="" SRC="../wp-content/uploads/2015/10/iniciar-la-reproduccion.png" width="30px"/></a>
            </td>
        ...
        <td width="50px"><a style="color:black" onclick="EditarFormacionConfiguracion(<?php echo $iddiplomado ?>)" ><img title="Editar" alt="" SRC="../wp-content/uploads/2015/10/System-icon.png" width="30px"/></a></td>
