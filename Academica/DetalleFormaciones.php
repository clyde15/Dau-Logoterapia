<html lang="es-ES" xml:lang="es-ES">
 ...
 $user = $_GET["idformaciones"];

 $query = "SELECT F.Nombre,T.Nombre as 'Tipo de Formacion',C.AlumnosInscritos,C.AlumnosRetirados,C.AlumnosTraslados,CONCAT(P.Nombre,' ',P.Apellido) as Profesor,GROUP_CONCAT(DS.Nombre SEPARATOR ', ') as Dias  FROM logoterapia.formacion F INNER JOIN logoterapia.configuraciones c ON c.ID_formacion = F.idformaciones INNER JOIN logoterapia.tipo T on T.idTipo = C.ID_Tipo INNER JOIN logoterapia.profesores P ON C.ID_ProfesorInicial = P.idprofesores INNER JOIN logoterapia.participacion PT ON PT.ID_Configuracion = C.IDConfiguracion INNER JOIN logoterapia.diassemana DS ON PT.ID_DiasSemana = DS.idDiasSemana WHERE F.idformaciones = '$user' ";
...
<td><?php echo $registros['Tipo de Formacion'] ?> </td>
