    ...
    <thead>
        ...
        <tr>
           <th>
            ...
            <th>Descripcion de la Formación</th>
...
<tbody>
   <?php   
   if ($Est == 1) {
       $query = "SELECT idformaciones,Nombre,Descripcion,DATE_FORMAT(FechaInicio,'%d-%m-%Y') as FechaInicio,DATE_FORMAT(FechaFin,'%d-%m-%Y') as FechaFin FROM logoterapia.formacion F INNER JOIN logoterapia.configuraciones C ON C.ID_Formacion = F.idformaciones WHERE F.Activo = 1 AND F.Eliminado = 0";
   }  else {
       $query = "SELECT idformaciones,Nombre,Descripcion,DATE_FORMAT(FechaInicio,'%d-%m-%Y') as FechaInicio,DATE_FORMAT(FechaFin,'%d-%m-%Y') as FechaFin FROM logoterapia.formacion F INNER JOIN logoterapia.configuraciones C ON C.ID_Formacion = F.idformaciones WHERE F.Activo = 1 AND F.Eliminado = 0 AND C.FechaInicio BETWEEN DATE_SUB(CURDATE(), INTERVAL 4 WEEK) AND CURDATE()";
    }
    ...
    while( $registros = $resultados->fetch_array()){
     ?>
      <tr>
         ...
         <td width="50px"><a style="color:black" href="#DetalleF" onclick="DetalleFormaciones(<?php echo $registros['idformaciones']; ?>)" ><img title="Detalle" alt="" SRC="../wp-content/uploads/2015/10/detalle.png" width="30px"/></a></td>
        <td width="50px"><a style="color:black" href="#DetalleD" onclick="ListaAlumnos(<?php echo $registros['idformaciones'].",'F'"; ?>)" ><img title="Alumnos Inscritos" alt="" SRC="../wp-content/uploads/2015/10/deberes.png" width="30px"/></a></td>            
